...
exports.compileIndexPage = function(options = {}) {
  const fileContents = fs.readFileSync(`${options.static}/index.html`, 'utf-8');
  const redocConfig = readYamlOrDefault(
    `${options.static}/redoc-config.yaml`,
    {},
    `ReDoc config not found in ${chalk.yellow(`${options.static}/redoc-config.yaml`)}. Skipping...`
  );
...
exports.specMiddleware = function(options = {}) {
  ...
  router.get('/', function(req, res) {
    try {
      const page = exports.compileIndexPage(Object.assign({}, options, { livereload: true }));
      res.end(page);
    } catch (e) {
      console.log(chalk.red(e.message));
      res.writeHead(500, {
  ...
  router.use(express.static(options.static || 'web'));
  return router;
};
...
function baseName(path) {
    console.log('baseName', path, Path.parse(path).name);
